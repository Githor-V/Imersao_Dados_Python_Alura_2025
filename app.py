...
#Configuração da página - define o título, o ícone e o layout
st.set_page_config(
...
if not df_filtrado.empty:
    ...
    cargo_mais_frequente = df_filtrado["titulo_cargo"].mode()[0]
else:
...
with col_graf1:
    if not df_filtrado.empty:
        top_cargos = df_filtrado.groupby('titulo_cargo')['salario_em_dolar'].mean().nlargest(10).sort_values(ascending=True).reset_index()
        grafico_cargos = px.bar(
            ...
            labels={'salario_em_dolar': 'Média salarial anual (USD)', 'titulo_cargo': ''}
        )
...
with col_graf4:
    if not df_filtrado.empty:
        df_ds = df_filtrado[df_filtrado['titulo_cargo'] == 'Data Scientist']
